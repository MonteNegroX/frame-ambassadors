...
interface LiquidGlassCardProps {
  ...
  transition?: any;
  shimmer?: boolean;
  [key: string]: any; // Allow for other HTML props
}
...
export const LiquidGlassCard = ({
  ...
  const MotionComponent = draggable || expandable || layoutId ? motion.div : 'div';

  const isDraggable = draggable === true || draggable === "true";

  const motionProps =
    isDraggable || expandable || layoutId
      ? {
          layoutId,
          transition,
          variants: (expandable ? containerVariants : undefined) as any,
          animate: expandable
            ? isExpanded
              ? 'expanded'
              : 'collapsed'
          ...
          dragConstraints: isDraggable
            ...
            : undefined,
          dragElastic: isDraggable ? 0.3 : undefined,
          dragTransition: isDraggable
            ? {
                bounceStiffness: 300,
                bounceDamping: 15,
                power: 0.1,
          ...
          whileDrag: isDraggable ? { 
            scale: 1.03, 
            zIndex: 50,
            boxShadow: '0 0 30px #facc1533' 
          } : undefined,
          whileHover: { 
            scale: 1.02, 
            boxShadow: '0 0 20px #facc151a',
          },
          whileTap: { scale: 0.97 },
        }
      : {};
  ...
  return (
    <>
      ...
      <MotionComponent
        className={cn(
          `relative isolate overflow-hidden ${isDraggable ? 'cursor-grab active:cursor-grabbing' : ''} ${expandable ? 'cursor-pointer' : ''}`,
          "transition-all duration-500 ease-out hover:bg-[#facc150d] hover:border-[#facc1533]",
          className
        )}
        style={{
          borderRadius,
      ...
      >
        ...
        <div
          className={`absolute inset-0 ${blurClasses[blurIntensity]} -z-30`}
          style={{
            borderRadius,
        ...
        {shimmer && (
          <motion.div 
            ...
            style={{
              background: "linear-gradient(90deg, transparent, rgba(250, 204, 21, 0.03), rgba(250, 204, 21, 0.1), rgba(250, 204, 21, 0.03), transparent)",
              width: "50%",
              skewX: "-20deg"
            }}
          />
