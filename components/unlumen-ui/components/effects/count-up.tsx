...
function OdometerDigit({ springValue, place }: OdometerDigitProps) {
  ...
  const y = useTransform(springValue, (v) => {
    ...
    const integerPart = Math.floor(val / place) % 10;
    const remainder = val % place;
    // Only start rolling when the remainder is in the final 10% of the current digit's range
    const roll = Math.max(0, (remainder / place) * 10 - 9);
    const digit = (integerPart + roll) % 10;
